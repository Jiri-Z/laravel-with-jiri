...
<div>
    ...
    @else
        <div class="flex items-center justify-between">
            ...
            <button wire:click="restart" wire:loading.attr="disabled" wire:target="restart" class="inline-flex items-center gap-2 px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-full font-semibold text-sm text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 shadow-sm hover:bg-gray-50 dark:hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
                <span wire:loading.remove wire:target="restart">{{ __('steps.quiz_restart') }}</span>
                <span wire:loading wire:target="restart" class="flex items-center gap-2">
                    <svg class="animate-spin h-4 w-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                        <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                        <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"></path>
                    </svg>
                    {{ __('steps.quiz_restarting') }}
                </span>
            </button>
        </div>
    @endif
