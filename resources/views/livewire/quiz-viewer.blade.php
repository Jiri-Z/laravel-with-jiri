...
<div>
    ...
    @else
        <div class="mb-6">
            ...
            @elseif ($step->type === StepType::QuizText)
                <div>
                    <textarea wire:model="textAnswer" rows="3" class="w-full rounded-xl border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-white focus:border-indigo-500 focus:ring-indigo-500" {{ $submitted ? 'disabled' : '' }}></textarea>
                </div>
            @endif
        </div>
    @endif
    ...
    @if (!$submitted)
        <button wire:click="submit" class="inline-flex items-center px-5 py-2.5 bg-indigo-600 border border-transparent rounded-full font-semibold text-sm text-white shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
            {{ $step->type === StepType::Quiz ? 'Submit All Answers' : 'Submit Answer' }}
        </button>
    @else
        <div class="flex items-center gap-2 {{ $isCorrect ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400' }}">
            ...
            @else
                ...
                <span class="font-medium">{{ $step->type === StepType::Quiz ? 'Some answers incorrect' : 'Incorrect' }}</span>
            @endif
        </div>
    @endif
</div>
