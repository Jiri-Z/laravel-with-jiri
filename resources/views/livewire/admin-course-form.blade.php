    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white dark:bg-gray-750 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <h1 class="text-2xl font-bold text-gray-900 dark:text-white mb-6">{{ $course ? __('admin.edit_course') : __('admin.create_course') }}</h1>
                    ...
                    <form wire:submit="save" class="space-y-6">
                        ...
                        <div class="flex items-center gap-3">
                            <button type="submit" class="inline-flex items-center px-4 py-2 bg-indigo-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
                                {{ $course ? __('admin.submit_update', ['resource' => __('admin.resource_course')]) : __('admin.submit_create', ['resource' => __('admin.resource_course')]) }}
                            </button>
                            <a href="{{ route('admin.courses.index') }}" wire:navigate class="text-sm text-gray-600 dark:text-gray-400 hover:underline">{{ __('admin.cancel') }}</a>
                        </div>
                    </form>
                </div>
            </div>
